...
const FolderComponent: React.FC<FolderComponentProps> = ({
  ...
  const handleFolderDoubleClick = (
    doubleClickedFolderId: string,
    doubleClickedFolderName: string
  ) => {
    onFolderDoubleClick &&
      onFolderDoubleClick(doubleClickedFolderId, doubleClickedFolderName);
  };
  ...
  const renderFolderList = (head: { [key: string]: any }) => {
    ...
    const sortedFolders = folders
      .filter((folder: Folder) => folder.createdAt)
      ...
      items.push(
        selectFolder ? (
          <ListItem
            ...
            onDoubleClick={() =>
              handleFolderDoubleClick(folder.folderId, folder.folderName)
            }
          ...
          <ListItem
            key={folder.folderId}
            button
            onClick={() => navigate(`/user/folders/${folder.folderId}`)}
          >
            ...
            <ListItemSecondaryAction>
              <IconButton
                edge="end"
                aria-label="Edit"
                onClick={() => handleEditClick(folder.folderId)}
              >
                <EditIcon />
              </IconButton>
            </ListItemSecondaryAction>
  ...
  return (
    <>
      <List>{renderFolderList(folder)}</List>
